...
if (window.app = {
    ...
    init: function() {
        new HeaderView, new MenuView, document.addEventListener("deviceready", function() {
            Backbone.history.start(), app.ready()
        }, !1)
    },
    ...
    afterRender: function() { // only show footer on index view
        Backbone.history.fragment.length && $("footer").empty()
    },
    ready: function() {
        app.checkConnection(), document.addEventListener("resume", reLoginOnResume, !1), document.addEventListener("resume", startLocationWatch, !1), document.addEventListener("pause", clearLocationWatch, !1)
    },
    getGeoPositionLatLon: function() {
        var lGeoCoords = {};
        return navigator.geolocation.getCurrentPosition(function(position) {
            lGeoCoords.latitude = position.latitude, lGeoCoords.longitude = position.longitude
        }, function() {
            lGeoCoords.latitude = 0, lGeoCoords.longitude = 0
        }, {
            enableHighAccuracy: !0
        }), lGeoCoords
    },
    showDialog: function(aTitle, aText, aDissmissText) {
        navigator.notification.alert(aText, // message
    ...
    getCityState: function() {
        var lCity, lState;
        return $.getJSON("http://nominatim.openstreetmap.org/reverse?format=json&lat=" + gLatitude.toFixed(3) + "&lon=" + gLongitude.toFixed(3), function(data) {
            lCity = data.address.city, lState = data.address.state
        }), {
            city: lCity,
            state: lState
        }
    ...
    checkConnection: function() {
        var lNetworkState = navigator.connection.type,
            lStates = {};
        lStates[Connection.UNKNOWN] = "Unknown connection", lStates[Connection.ETHERNET] = "Ethernet connection", lStates[Connection.WIFI] = "WiFi connection", lStates[Connection.CELL_2G] = "Cell 2G connection", lStates[Connection.CELL_3G] = "Cell 3G connection", lStates[Connection.CELL_4G] = "Cell 4G connection", lStates[Connection.NONE] = "No network connection";
        var lConnectState = lStates[lNetworkState];
        ("Unknown connection" === lConnectState || "No network connection" === lConnectState) && app.showDialog("Network Error!", "Finsight is having trouble connecting to the network.", "Ok")
    },
    menuToggle: function() {
        var lSpeed = 200,
            lMenuWidth = 248,
        ...
        lMenu.hasClass("open") ? (lToggler.removeClass("active"), lMenu.removeClass("open").animate({
            left: "-" + lMenuWidth + "px"
        }, lSpeed), lInner.animate({
            "margin-left": 0
        }, lSpeed), lFixedElements.animate({
            left: 0
        }, lSpeed)) : (lToggler.addClass("active"), lMenu.addClass("open").animate({
            left: 0,
    ...
    resize: function() {
        function resizeCallback() {
            resizeTimeoutId && window.clearTimeout(resizeTimeoutId), resizeTimeoutId = window.setTimeout(resizeAction, 250)
        }

        function resizeAction() { //console.log($(window).width(), $(window).height())
            callback()
        }
    ...
    app.router = Backbone.Router.extend({
        ...
        initialize: function() {
            Backbone.history.start(), app.init()
        },
        index: function() {
            app.preRoute(this.el), new IndexView({
                el: this.el
            })
        },
        login: function() {
            app.preRoute(this.el), new LoginView({
                el: this.el
            })
        },
        mySpots: function() {
            app.preRoute(this.el), new MySpotsView({
                el: this.el
            })
        }
...
if (
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // ALERT PLUGIN DEFINITION
        // =======================
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // BUTTON PLUGIN DEFINITION
        // ========================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
        ...
        Button.VERSION = "3.2.0", Button.DEFAULTS = {
            ...
            var d = "disabled",
                ...
                data = $el.data();
            state += "Text", null == data.resetText && $el.data("resetText", $el[val]()), $el[val](null == data[state] ? this.options[state] : data[state]), // push to event loop to allow forms to submit
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // CAROUSEL PLUGIN DEFINITION
        // ==========================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
        ...
        Carousel.VERSION = "3.2.0", Carousel.DEFAULTS = {
            ...
            var that = this,
                activeIndex = this.getItemIndex(this.$active = this.$element.find(".item.active")); // yes, "slid"
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // COLLAPSE PLUGIN DEFINITION
        // ==========================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // MODAL PLUGIN DEFINITION
        // =======================
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // TOOLTIP PLUGIN DEFINITION
        // =========================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
        ...
        Tooltip.VERSION = "3.2.0", Tooltip.DEFAULTS = {
            ...
            var $tip = this.tip(),
                ...
                marginLeft = parseInt($tip.css("margin-left"), 10); // we must check for NaN for ie 8/9
            isNaN(marginTop) && (marginTop = 0), isNaN(marginLeft) && (marginLeft = 0), offset.top = offset.top + marginTop, offset.left = offset.left + marginLeft, // $.fn.offset doesn't round pixel values
        ...
        }, Tooltip.prototype.getCalculatedOffset = function(placement, pos, actualWidth, actualHeight) { /* placement == 'right' */
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // POPOVER PLUGIN DEFINITION
        // =========================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
        ...
        Popover.prototype = $.extend({}, $.fn.tooltip.Constructor.prototype), Popover.prototype.constructor = Popover, Popover.prototype.getDefaults = function() {
            ...
            $tip.find(".popover-title")[this.options.html ? "html" : "text"](title), $tip.find(".popover-content").empty()[ // we use append for html objects to maintain js events
                this.options.html ? "string" == typeof content ? "html" : "append" : "text"](content), $tip.removeClass("fade top bottom left right in"), // IE8 doesn't accept hiding via the `:empty` pseudo selector, we have to do
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // TAB PLUGIN DEFINITION
        // =====================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
        ...
        Tab.VERSION = "3.2.0", Tab.prototype.show = function() {
            ...
            function next() {
                $active.removeClass("active").find("> .dropdown-menu > .active").removeClass("active"), element.addClass("active"), transition ? (element[0].offsetWidth, // reflow for transition
                    element.addClass("in")) : element.removeClass("fade"), element.parent(".dropdown-menu") && element.closest("li.dropdown").addClass("active"), callback && callback()
            }
     ...
     * ======================================================================== */
    + function($) {
        "use strict"; // AFFIX PLUGIN DEFINITION
        // =======================
        function Plugin(option) {
            return this.each(function() {
                var $this = $(this),
...
}(jQuery), /*! jQuery v1.11.1 | (c) 2005, 2014 jQuery Foundation, Inc. | jquery.org/license */ ! function(a, b) {
...
function(a) {
    function g(e, d, g, i) {
        ...
        return e && a.extend(c, e, {
            nodes: [],
            parent: d
        }), g && (c.tmpl = g, c._ctnt = c._ctnt || c.tmpl(a, c), c.key = ++h, (l.length ? f : b)[h] = c), c
    }

    function i(e, g, f) {
        var b, c = f ? a.map(f, function(a) {
            return "string" == typeof a ? e.key ? a.replace(/(<\w+)(?=[\s>])(?![^>]*_tmplitem)([^>]*)/g, "$1 " + d + '="' + e.key + '" $2') : a : i(a, e, a._ctnt)
        }) : e;
        return g ? c : (c = c.join(""), c.replace(/^\s*([^<\s][^<]*)?(<[\w\W]+>)([^>]*[^>\s])?\s*$/, function(f, c, e, d) {
            b = a(e).get(), m(b), c && (b = j(c).concat(b)), d && (b = b.concat(j(d)))
        }), b ? b : j(c))
    }
    ...
    function o(b) {
        return new Function("jQuery", "$item", "var $=jQuery,call,_=[],$data=$item.data;with($data){_.push('" + a.trim(b).replace(/([\\'])/g, "\\$1").replace(/[\r\t\n]/g, " ").replace(/\$\{([^\}]*)\}/g, "{{= $1}}").replace(/\{\{(\/?)(\w+|.)(?:\(((?:[^\}]|\}(?!\}))*?)?\))?(?:\s+(.*?)?)?(\(((?:[^\}]|\}(?!\}))*?)\))?\s*\}\}/g, function(m, l, j, d, b, c, e) {
            var h, f, g, i = a.tmpl.tag[j];
            if (!i) throw "Template command not found: " + j;
            return h = i._default || [], c && !/\w$/.test(b) && (b += c, c = ""), b ? (b = k(b), e = e ? "," + k(e) + ")" : c ? ")" : "", f = c ? b.indexOf(".") > -1 ? b + c : "(" + b + ").call($item" + e : b, g = c ? f : "(typeof(" + b + ")==='function'?(" + b + ").call($item):(" + b + "))") : g = f = h.$1 || "null", d = k(d), "');" + i[l ? "close" : "open"].split("$notnull_1").join(b ? "typeof(" + b + ")!=='undefined' && (" + b + ")!=null" : "true").split("$1a").join(g).split("$1").join(f).split("$2").join(d ? d.replace(/\s*([^\(]+)\s*(\((.*?)\))?/g, function(d, c, b, a) {
                return a = a ? "," + a + ")" : b ? ")" : "", a ? "(" + c + ").call($item" + a : d
            }) : h.$2 || "") + "_.push('"
        }) + "');}return _;")
    }

    function n(c, b) {
        c._wrap = i(c, !0, a.isArray(b) ? b : [q.test(b) ? b : a(b).html()]).join("")
    }
    ...
    function m(o) {
        function m(j) {
            function o(a) {
                a += n, e = l[a] = l[a] || g(e, b[e.parent.key + n] || e.parent, null, !0)
            }
            var p, k, e, m, i = j;
            if (m = j.getAttribute(d)) {
                for (; i.parentNode && 1 === (i = i.parentNode).nodeType && !(p = i.getAttribute(d)););
                p !== m && (i = i.parentNode ? 11 === i.nodeType ? 0 : i.getAttribute(d) || 0 : 0, (e = b[m]) || (e = f[m], e = g(e, b[i] || f[i], null, !0), e.key = ++h, b[h] = e), c && o(m)), j.removeAttribute(d)
            } else c && (e = a.data(j, "tmplItem")) && (o(e.key), b[e.key] = e, i = a.data(j.parentNode, "tmplItem"), i = i ? i.key : 0); if (e) {
                for (k = e; k && k.key != i;) k.nodes.push(j), k = k.parent;
                delete e._ctnt, delete e._wrap, a.data(j, "tmplItem", e)
            }
        }
        var k, j, e, p, i, n = "_" + c,
            l = {};
        for (e = 0, p = o.length; p > e; e++)
            if (1 === (k = o[e]).nodeType) {
                for (j = k.getElementsByTagName("*"), i = j.length - 1; i >= 0; i--) m(j[i]);
                m(k)
            }
    }

    function u(a, d, c, b) {
        return a ? void l.push({
            _: a,
            tmpl: d,
            item: this,
            data: c,
    ...
    a.each({
        ...
        a.fn[f] = function(n) {
            ...
            if (e = b || {}, j && 11 === j.nodeType && 1 === j.childNodes.length && 1 === i.length) i[d](this[0]), g = this;
            else {
                for (h = 0, m = i.length; m > h; h++) c = h, k = (h > 0 ? this.clone(!0) : this).get(), a.fn[d].apply(a(i[h]), k), g = g.concat(k);
        ...
        domManip: function(d, l, j) {
            if (d[0] && d[0].nodeType) {
                for (var h, f = a.makeArray(arguments), g = d.length, i = 0; g > i && !(h = a.data(d[i++], "tmplItem")););
                g > 1 && (f[0] = [a.makeArray(d)]), h && c && (f[2] = function(b) {
        ...
        tmpl: function(d, h, e, c) {
            var j, k = !c;
            if (k) c = p, d = a.template[d] || a.template(null, d), f = {};
            else if (!d) return d = c.tmpl, b[c.key] = c, c.nodes = [], c.wrapped && n(c, c.wrapped), a(i(c, null, c.tmpl(a, c)));
            return d ? ("function" == typeof h && (h = h.call(c || {})), e && e.wrapped && n(e, e.wrapped), j = a.isArray(h) ? a.map(h, function(a) {
                return a ? g(e, c, d, a) : null
            }) : [g(e, c, d, h)], k ? a(i(c, null, j)) : j) : []
        },
        tmplItem: function(b) {
            var c;
        ...
        template: function(c, b) {
            return b ? ("string" == typeof b ? b = o(b) : b instanceof a && (b = b[0] || {}), b.nodeType && (b = a.data(b, "tmpl") || a.data(b, "tmpl", o(b.innerHTML))), "string" == typeof c ? a.template[c] = b : b) : c ? "string" != typeof c ? a.template(null, c) : a.template[c] || a.template(null, q.test(c) ? c : a(c)) : null
        },
        encode: function(a) {
...
function() {
...
}.call(this), FooterView = Backbone.View.extend({
    el: "footer",
    events: {},
    initialize: function() {
        app.preRoute(this.$el), this.render()
    },
    render: function() { //var model = this.create_view_model(),
        var lHtml = $("#tmpl-footer").tmpl();
        this.$el.html(lHtml), $(".footer-shadow").show()
    }
    ...
    menuToggle: function(aEvent) {
        return app.menuToggle(aEvent), !1
    }
}), IndexView = Backbone.View.extend({
    events: {},
    initialize: function() {
        var that = this;
        that.render(), that.afterRender(),
        /*GlobalModel('stupiddeal', function (model) {
            var facebookUrl = that.getFacebookUrl;
            model.attributes.resultset.facebookUrl = facebookUrl;
            that.render(model);
            app.afterRender();
            new FooterView(model);
        });*/
        new FooterView
    },
    render: function() {
    ...
    initialize: function() {
        var that = this;
        that.render(), app.afterRender(), that.afterRender()
    },
    back: function() {
    ...
    render: function(model) {
        var html = (model && model.get("resultset") || {}, $("#tmpl-login").tmpl());
        this.$el.html(html)
    },
    afterRender: function() {
        app.changePageTitle("Login"), $(".left-nav").show(), $("#menuToggle").hide()
    }
    ...
    menuToggle: function(event) {
        ...
        $(event.currentTarget).hasClass("hassub") || app.menuToggle(!0)
    },
    exit: function() {
        return navigator.app.exitApp(), !1
    }
}), MyProfileView = Backbone.View.extend({
    events: {
        "click .click-test": "testLoader"
    },
    initialize: function() {
        var that = this;
        that.render(), app.afterRender(), app.changePageTitle("My Profile")
    },
    testLoader: function(e) {
        app.showLoader(e)
    },
    render: function(model) {
        var html = (model && model.get("resultset") || {}, $("#tmpl-myprofile").tmpl());
        this.$el.html(html)
    }
}), MySpotsView = Backbone.View.extend({
    events: {
        "click .click-test": "testLoader"
    },
    initialize: function() {
    ...
    render: function(model) {
        var html = (model && model.get("resultset") || {}, $("#tmpl-myspots").tmpl());
        this.$el.html(html)
    },
    afterRender: function() {
        app.changePageTitle("My Spots"), $(".left-nav").show(), $("#menuToggle").hide()
    }
    ...
    return show_loader !== !1 && app.showLoader(), _.isNull(navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) ? ($.mockjax({
        url: jsonPath,
        responseTime: 750,
        ...
        model.get("resultset").Error ? new ErrorView({
            model: model
        }) : callback(model), app.hideLoader()
    })) : $.get("http://google.com/m/" + jsonPath + ".php", function(response) {
        model.set({
            resultset: response.resultset
        }), callback(model)
    }).fail(function() { //alert(JSON.stringify(response))
        model.set({
            resultset: {
                Error: defaultError
            }
